...
- 为什么不推荐 《虚拟机 + VeraCrypt》的方案，而推荐macbook作为反取证的首选方案？
  ...
  - 其次，虚拟机内运行操作系统，也对性能（尤其是IO性能）有很大损耗，无法释放100%性能。加上VeraCrypt就是双重性能损耗。远远不如Macbook。

  - 再次，如果虚拟机内外，安装的都是Linux，则更过于复杂，小白用户非常不友好，不如Macbook

  - 再次，Linux并非原生支持加密，必须配合各种第三方软件（虚拟机和Veracrypt）才能实现反取证。这比Win11配置反取证还复杂（尤其GPU驱动、各种硬件驱动、硬件映射，想想 瞬间头都大了），而且对性能损耗过大。
...
- 扛不住大记忆恢复术怎么办？
  ...
  - 对于iPhone：
    - 核心在于“骗”。颜悦色的 “骗” 非法取证的人，让他们输错10次解锁密码。然后手机直接自动销毁数据，让他们哭去吧。🤣🤣

  - 对于MacBook：
    - 只能期望苹果尽早加入上述功能。

  - 对于Win11：
    - 因为没人能记住48位Bitlocker密码。所以如果你使用了带有bitlocker intrusion detection的电脑，一旦系统检测到机箱被拆解，直接弹窗48位密钥输入窗口。一劳永逸解决战斗。

  总之，对抗大记忆恢复术，我一直认为，最最最一劳永逸的办法，永远是，直接销毁数据。而不是别的骚操作（让你解不开或看不见）。相信未来，基于条件触发的直接原地销毁数据 + 解锁前无法克隆硬盘，会是大家殊途同归的保底方案。